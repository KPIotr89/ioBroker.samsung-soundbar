...
const COMMAND_KEYS = [ 'power', 'volume', 'mute', 'input', 'inputNum', 'soundMode', 'soundModeNum', 'remoteKey' ];
const NUMERIC_KEYS = [ 'volume', 'inputNum', 'soundModeNum' ];
const BOOL_FORMATS = {
...
class MqttBridge {
    ...
    /** Accept "host", "host:1883" and full URLs alike - mqtt.js needs a scheme. */
    static normaliseUrl(raw) {
        const url = String(raw || '').trim();
        if (!url) {
            return '';
        }
        return /^[a-z]+:\/\//i.test(url) ? url : `mqtt://${url}`;
    }

    start() {
        const url = MqttBridge.normaliseUrl(this.config.mqttUrl);
        if (!url) {
            throw new Error('MQTT broker address is empty - fill it in or switch the bridge off');
        }
    ...
    _dispatch(key, rawValue) {
        ...
        if (key === 'power' || key === 'mute') {
            ...
            if (!Number.isFinite(value)) {
                this.log.warn(`MQTT: cannot parse number "${rawValue}" for ${key}`);
                return;
            }
        } else {
...
module.exports = { MqttBridge, COMMAND_KEYS, NUMERIC_KEYS, parseBool };
